                               ...
                               oc_interface_mask_t iface_mask, void *data)
{
  ...
  if (valid_request == SPAKE_RND) {
    ...
    oc_new_byte_string(&g_pase.id, "responderkey", strlen("responderkey"));
  }
  // handle input
  while (rep != NULL) {
    switch (rep->type) {
...
oc_core_knx_spake_separate_post_handler(void *req_p)
{
  ...
  if (valid_request == SPAKE_RND) {
    ...
    oc_oscore_set_auth_device(oc_string(device->serialnumber), oc_string_len(device->serialnumber), oc_string(g_pase.id),
                       oc_byte_string_len(g_pase.id), shared_key,
                       (int)shared_key_len);
