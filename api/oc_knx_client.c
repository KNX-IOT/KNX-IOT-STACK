...
{
  oc_oscore_set_auth(oc_string(g_spake_ctx.serial_number), g_spake_ctx.oscore_id, secret, secret_size);
}
...
finish_spake_handshake(oc_client_response_t *data)
{
  ...
  if (m_spake_cb) {
    ...
    m_spake_cb(0, oc_string(data->endpoint->serial_number), g_spake_ctx.oscore_id,
               shared_key,
               shared_key_len);
  }
}
