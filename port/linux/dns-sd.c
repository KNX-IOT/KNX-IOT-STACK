...
knx_publish_service(char *serial_no, uint32_t iid, uint32_t ia, bool pm)
{
  ...
  if (avahi_pid == 0) {
    ...
    snprintf(serial_no_hostname, sizeof(serial_no_hostname), "knx-%s", serial_no);
    ...
    if (pm) {
      ...
      if (!iid || !ia) {
        error = execlp("avahi-publish-service", "avahi-publish-service",
                       serial_no_subtype, serial_no_hostname, "_knx._udp", port_str,
                       (char *)NULL);
      } else {
        // --subtype=_ia3333-CA._sub._knx._udp
        char *format_string = "--subtype=_ia%x-%x._sub._knx._udp";
        ...
        error = execlp("avahi-publish-service", "avahi-publish-service",
                       serial_no_subtype, installation_subtype, serial_no_hostname,
